...
def plot(data, low, upper, xran, color, label):
    plt.plot(xran, data, lw = 1, color=color, alpha=1,label=label)
...
plot(means, means-stds*0.9, means+stds*0.9, xran, 'y', 'GAIL_action_proxies')
...
plot([means]*nums, [means-stds*0.9]*nums, [means+stds*0.9]*nums, xran, 'g', 'Expert')
...
k = nums*2048
#xran = np.arange(200) * 2048
plot(means, means-stds*0.9, means+stds*0.9, xran, 'k', 'GAIfO')

fpath = './mpic1_/'
plt.title(envs[t])
plt.xlabel('interactions: 10^x')
plt.ylabel('episodic returns')
plt.savefig(fpath+envs[t]+'_n'+vv+'.pdf', format='pdf', dpi=1200, transparent=True, bbox_inches='tight', pad_inches=0.05)
plt.show()
